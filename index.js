...
const input = document.getElementById('glyphInput');
        ...
        const tableBody = document.querySelector('#fishTable tbody');
        const totalQuantityEl = document.getElementById('totalQuantity');
        const totalWeightEl = document.getElementById('totalWeight');
        const totalScoreEl = document.getElementById('totalScore');

        // Class multipliers inspired by NMS rarity (Common:1, Uncommon:2, Rare:3)
        ...
        // Array to store fish entries for easy recalculation
        let fishEntries = [];
        ...
        form.addEventListener('submit', (e) => {
            ...
            const fishClass = document.getElementById('fishClass').value;
            const weight = parseFloat(document.getElementById('fishWeight').value);
            const quantity = parseInt(document.getElementById('fishQuantity').value);

            // Add to entries
            fishEntries.push({ name, fishClass, weight, quantity });
        ...
        function updateTable() {
            ...
            fishEntries.forEach(entry => {
                ...
                const totalEntryWeight = entry.weight * entry.quantity;
                const score = totalEntryWeight * classMultipliers[entry.fishClass]; // Score = total weight * class multiplier

                row.innerHTML = `
                    <td>${entry.name}</td>
                    <td>${entry.fishClass}</td>
                    <td>${entry.weight.toFixed(1)}</td>
                    <td>${entry.quantity}</td>
                    <td>${totalEntryWeight.toFixed(1)}</td>
